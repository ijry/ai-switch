...
`冷却 N 秒` 徽章是个按钮，点一下弹出「账号冷却」对话框：

- **剩余冷却（秒）**：打开时按数据库里真实剩余的秒数填好，所以什么都不动直接保存不会偷偷延长等待。旁边五个一键步进 `−5 分` `−1 分` `+1 分` `+5 分` `+30 分`，算出来越界会夹回 1 到 86,400 秒之间。保存后 `next_retry_at` 与 `cooldown_until` 一起被写成 `现在 + 这个秒数`，**失败次数与最近失败信息一列不动**——「少等一会儿」是调度决定，不等于账号已经恢复。
- **立即解除冷却**：等同于一次请求成功后的清账——冷却时间戳、`错误 N 次` 的计数、语义连击和最近失败信息全部清空，账号立刻回到轮换里。这是账号级的 `解除`，与模型级那个 `解除` 对称。

两个动作都**不碰模型行**：模型级的冷却与异常有它自己的 `解除`，把账号的退避清掉并不代表一小时前回 429 的那个模型也好了。冷却状态只存 SQLite、没有内存缓存，所以解除后的下一个请求立刻能选中这个账号。

::: tip 徽章只在冷却中出现
冷却过期后徽章自然消失，此时没有什么可解除的；剩下的 `错误 N 次` 会在下一次请求成功时自己清零。如果想连计数一起清掉，趁冷却还在时点「立即解除冷却」。
:::
...
::: tip 手动测试就是最快的恢复手段
针对单个账号点一次模型连通性测试，成功即执行完整恢复。`paused` 的账号也可以被测试——代码里明确注释了这一点：显式测一次正是用户判断暂停中的账号是否已恢复的方式。想解除单个模型的冷却或异常，编辑抽屉「故障处理」分区的「模型状态」里点一次 `解除` 更直接，不消耗配额。想解除**账号级**冷却，点账号行的 `冷却 N 秒` 徽章再点「立即解除冷却」，同样不消耗配额。
:::
