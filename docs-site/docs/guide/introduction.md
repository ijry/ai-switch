...
- **最大并发数** 默认 5，最小 1
- **失败处理策略**：额外重试次数、重试间隔、异常触发次数、失败冷却秒数
...
Web 服务默认绑定 `127.0.0.1:19527`。桌面设置可改为 `0.0.0.0` 直接向局域网开放，无需 TLS；此时全部 Web 命令都可通过明文 HTTP 调用，访问令牌是唯一保护。独立 server 仍默认拒绝非 loopback 明文监听。
