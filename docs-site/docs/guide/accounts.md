...
- 查到的余额显示成账号行上的徽标（`余额 $37.70`），悬浮可看套餐、已用、总额、到期、订阅与限流窗口，以及更新时间。余额 ≤ 0 时徽标变红，面板标记为不限额度时显示「余额 不限」。
- 编辑抽屉的分段器下方也有一枚徽标和「立即查询」，改配置后可以当场验证。

**不会自动轮询，也不会因为余额低改账号状态。** new-api 的这个接口有每 key 20 次 / 20 分钟的限流，而这里的对象是一整池账号，自动轮询很容易把每个号都撞上限流；查询失败或面板改字段也不该连带把账号踢出调度。真的额度不足时，转发失败的那条路径仍然会把账号置为异常，见 [稳定性与自动恢复](/guide/reliability)。
...
**账号级冷却在列表里就能改。** 账号行的 `冷却 N 秒` 徽章是个按钮，点开可以增减剩余秒数，也可以「立即解除冷却」把退避和错误计数一起清掉。
