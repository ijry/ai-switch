...
在「设置 → 插件管理」中可以启用或关闭内置插件：

- **SaaS 插件**：沿用原有激活流程。首次启用需要内测码；开关只控制插件和左侧 SaaS 导航，不删除用户、账务或站点数据。
- **生图插件**：默认关闭。启用后左侧导航显示「生图」；关闭后隐藏入口，但会话和本地图片资产会保留。开关保存在 `settings.json` 的 `image_generation_enabled` 字段中，下次启动仍然生效。

## 启动恢复

以下状态各自记住自己的值，桌面端启动时按需生效：

- **算力池路由接入**：在算力池顶部启用后，`web-service.json` 会记住 `routeAccessEnabled`，应用启动时自动恢复共享监听。
- **本地算力池代理**：`route-proxy-https.json` 仍保存旧版独立代理的恢复状态；当前共享监听路径由算力池顶部的路由控制驱动。
