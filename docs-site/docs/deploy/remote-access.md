...
桌面版不用 Tailscale 时，也可以在设置中选择 `0.0.0.0` 直接通过 HTTP 向局域网开放，不再强制要求 TLS。此模式会开放全部 Web 命令，令牌是唯一保护；自备 TLS 仍适合不可信网络。独立 server 的非环回明文限制不变。
...
- **绑 `0.0.0.0` 之前先想清楚。** 桌面版允许直接明文启动，并向同一网络开放全部 Web 命令；访问令牌是唯一保护。
