...
账号按 **1–5 的优先级**参与调度，**默认 3**，数字小的先用。每个账号还有**并发上限，默认 1**——也就是说默认情况下一个账号同时只处理一个请求，第二个请求会去找池里的下一个账号。
...
::: tip 为什么并发上限默认只有 1？
因为多数官方账号与第三方端点对并发很敏感：同一账号并发多路请求容易触发限流，甚至被判定为异常使用。默认 1 意味着 AI Switch 倾向于把并发**分散到多个账号**（这正是算力池的意义），而不是压在单个账号上。如果你确认某个账号的上游允许更高并发，可以单独调高它的上限。
:::
...
**AI Switch 兼容 cc-switch 的导入协议**，目的只有一个：让使用 cc-switch 的用户能方便地把配置迁移过来。桌面端还可以选择性开启 `ccswitch://` 深链兼容（默认关闭），这样原本发给 cc-switch 的导入链接也能被 AI Switch 接收。

除此之外没有关系。AI Switch 是从零实现的独立项目：**本项目只研究公开行为、公开文档和公开文件格式，不复用其代码。** 仓库 README 的 "Clean-Room Boundary" 一节就是明确这条边界的。

换句话说，兼容体现在"能读懂同一种导入格式"这个层面，而不是共享实现。
