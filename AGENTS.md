...
3. 使用同一份双语更新日志作为 Git 提交信息，提交标题简洁明确，正文保留完整的中文和英文条目；不要另外创建持久化的 CHANGELOG 文件，除非用户明确要求。提交正文必须按「中文条目 → 空行 → 独占一行的 29 个连字符（`-----------------------------`）→ 空行 → 英文条目」的顺序排列：发布流水线把这条提交信息原样写入 GitHub Release 正文与更新清单的 `notes`，客户端据此按界面语言只显示对应语种的那一半。除该分隔线外，正文其余位置不要出现 12 个以上连续的 `-`、`*` 或 `_`。
