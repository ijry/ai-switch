...
import { MCP_APPS, appLabel } from "../components/mcp/catalog";
...
export function McpScreen() {
  ...
  const error = saveMutation.error ?? deleteMutation.error ?? installMutation.error ?? localQuery.error ?? marketQuery.error;
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-stone-200 pb-2">
        ...
        {view === "local" ? <button className="inline-flex items-center gap-1.5 bg-stone-900 px-2.5 py-1.5 text-[12px] font-semibold text-white hover:bg-stone-800" onClick={() => openEditor()} type="button"><Plus className="h-3.5 w-3.5" /> Add server</button> : null}
      </div>

      {view === "local" ? <LocalServers servers={localQuery.data ?? []} loading={localQuery.isLoading} onEdit={openEditor} onDelete={(id) => { if (window.confirm(`Remove ${id} from all clients?`)) deleteMutation.mutate(id); }} /> : (
        <MarketplaceView
