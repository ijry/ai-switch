    ...
    <section className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-stone-200 pb-2">
        <div className="inline-flex border border-stone-200 bg-white p-0.5" role="tablist">
          <button aria-selected={view === "local"} className={`px-3 py-1.5 text-[12px] font-semibold ${view === "local" ? "bg-stone-900 text-white" : "text-stone-600 hover:bg-stone-50"}`} onClick={() => setView("local")} role="tab" type="button">{t("mcp.localConfiguration")}</button>
          <button aria-selected={view === "market"} className={`px-3 py-1.5 text-[12px] font-semibold ${view === "market" ? "bg-stone-900 text-white" : "text-stone-600 hover:bg-stone-50"}`} onClick={() => setView("market")} role="tab" type="button">{t("mcp.marketplace")}</button>
        </div>
      ...
      {view === "local" ? <LocalServers filter={localSearch} servers={localQuery.data ?? []} loading={localQuery.isLoading} onEdit={openEditor} onDelete={(id) => { if (window.confirm(t("mcp.removeConfirm", { id }))) deleteMutation.mutate(id); }} /> : (
        <MarketplaceView
