  ...
  return (
    <>
      <header className="flex flex-wrap items-end justify-between gap-3 rounded-2xl border border-stone-200 bg-white/65 px-4 py-3 shadow-sm">
        ...
        <button className="inline-flex items-center gap-1.5 rounded-xl border border-stone-300 bg-white px-2.5 py-1.5 text-[12px] font-semibold text-stone-800 motion-control hover:bg-stone-50" onClick={props.onRefresh} title={t("skills.refreshTitle")} type="button">
          <RefreshCw className="h-3.5 w-3.5" /> {t("skills.refresh")}
        </button>
      </header>
      <div className="grid gap-2 rounded-2xl border border-stone-200 bg-white/55 p-3 shadow-sm lg:grid-cols-[minmax(180px,240px)_auto_minmax(180px,240px)_minmax(220px,1fr)_auto] lg:items-end">
        <label className="grid gap-1 text-[11px] font-semibold uppercase tracking-wide text-stone-400">
          <span>{t("skills.agent")}</span>
          <select className="rounded-xl border border-stone-300 bg-white px-2.5 py-2 text-[12px] font-medium normal-case tracking-normal text-stone-800 motion-control" onChange={(event) => props.onAgentChange(event.target.value as SkillAgentType)} value={props.agentType}>
        ...
        <div className="inline-flex rounded-xl border border-stone-200 bg-white p-0.5">
          <button className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold motion-control ${props.scope === "global" ? "bg-stone-900 text-white" : "text-stone-600 hover:bg-stone-50"}`} onClick={() => props.onScopeChange("global")} type="button">{t("skills.global")}</button>
        ...
        <label className="flex min-w-[180px] items-center gap-2 rounded-xl border border-stone-300 bg-white px-2.5 py-1.5">
        ...
        <button className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-stone-900 px-2.5 py-2 text-[12px] font-semibold text-white motion-control hover:bg-stone-800" onClick={props.onNew} type="button"><Plus className="h-3.5 w-3.5" /> {t("skills.new")}</button>
      </div>
    </>
  );
}
