...
describe("WebServiceSettings", () => {
  ...
  it("offers only loopback and all-interface hosts without a route-access setting", async () => {
    renderSettings();

    expect(await screen.findByRole("heading", { name: "共享服务端口" })).toBeInTheDocument();
    const host = await screen.findByRole("combobox", { name: "主机" });
    expect(
      within(host)
        .getAllByRole("option")
        .map((option) => option.getAttribute("value")),
    ).toEqual(["127.0.0.1", "0.0.0.0"]);
    expect(screen.queryByRole("heading", { name: "算力池路由接入" })).not.toBeInTheDocument();
    expect(screen.queryByRole("checkbox", { name: "启用算力池路由接入" })).not.toBeInTheDocument();
    expect(screen.queryByText("启动时自动运行")).not.toBeInTheDocument();

    await userEvent.selectOptions(host, "0.0.0.0");
